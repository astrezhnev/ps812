...
- No formal pre-requisites 
  - Math camp should get you up to speed on topics/refresh your memory of HS math.
...
- Sometimes it's valuable to *not* have the LLM **do** something so that you can learn to **think**.
...
- **Statistics** is an *extremely* recent field - hypothesis testing formalized in early 20th century
...
- **Political methodology** starts to define itself around 1984 (first PolMeth meeting)
...
- **Probability** quantifies the likelihood of an event from a given data-generating process
...
- Define the **sample space** $\Omega$ as the set of all possible outcomes of an "experiment"

::: {.fragment}
![](assets/pebble-world.svg){fig-align="center" width="40%"}

::: {.attribution}
Modeled after @blitzstein2019introduction, Figure 1.1
:::
:::
...
- An **event** is a **subset** of the sample space (including the empty set)
  - We'll denote the **event space** -- the set of all events that can be assigned a probability -- as $\mathcal{S}$ (this is called the "$\sigma$-algebra")
...
- The **complement** of a set $A$, denoted $A^{c}$, is the set of all elements not in $A$
  - Implicitly complements are defined w.r.t. some "universal" set - here the sample space $\Omega$.
  - What's $\Omega^c$?
...
- The **union** of two events $A \cup B$ contains all elements in **either** $A$ **or** $B$
  - We'll sometimes use a big $\displaystyle\bigcup_{i=1}^N$ to denote a union over *many* events 
- The **intersection** of two events $A \cap B$ contains all elements **both** in $A$ **and** $B$
  - Same idea for notation: $\displaystyle\bigcap_{i=1}^N$ denotes an intersection over *many* events 
...
- **Practice**: In the figure below, are $A$ and $B$ disjoint?
...
- Probability is a function that maps from the event space $\mathcal{S}$ to the real numbers.
  - $P \colon \mathcal{S} \to \mathbb{R}$
- The probability function $P$ obeys three axioms (*Kolmogorov's Axioms*)

  1) **Non-negativity**: $P(A) \geq 0$ for every event $A \in \mathcal{S}$
...
- If $A \subseteq B$, then $P(A) \leq P(B)$
  - A **bigger** event can't be **less** likely
- **Proof**
...
- Every event is a subset of the sample space: $A \subseteq \Omega$, so $P(A) \leq P(\Omega) = 1$
  - With non-negativity, this pins probabilities to the interval $[0, 1]$
...
::: {.fragment}
![](assets/inclusion-exclusion.svg){fig-align="center" width="80%"}
:::
...
- Let's inspect our expression for $P(A \cap B)$
  $$ P(A \cap B) = P(A) + P(B) - P(A \cup B) $$
- We have $P(A) + P(B)$...but what about $P(A \cup B)$
  - $A \cup B \subseteq \Omega$ and $P(\Omega) = 1$. So $P(A \cup B) \leq 1$
...
## Bounding the probability of a union
...
## Fréchet inequalities for two events
...
- Conditional probabilities
  - How do the chances of one event change if I **know** another event has occurred?
  - Conditioning **restricts the sample space**
- Independence
  - What happens when knowing one event **tells me nothing** about the probability of another?
- Bayes' rule
  - How do we relate the probability of $A$ given $B$ to the probability of $B$ given $A$?
