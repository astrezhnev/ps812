...
- The **union** of two events $A \cup B$ contains all elements in **either** $A$ **or** $B$
  - We'll sometimes use a $\displaystyle\bigcup_{i=1}^N$ to denote a union over *many* events 
...
## Inclusion/Exclusion {.smaller}

:::: {.incremental}

- The same idea extends to more events: **add** the pieces, then fix the double-counting
  - Each region below is numbered by how often the *first* sum counts it

::: {.fragment}
![](assets/inclusion-exclusion.svg){fig-align="center" width="80%"}
:::

- **Two events**: one region is counted twice, so subtract it once

  $$P(A \cup B) = P(A) + P(B) - P(A \cap B)$$

- **Three events**: subtract the three pairwise overlaps -- but that strips the centre
  entirely, so **add it back**
...
- We have $P(A) + P(B)$...but what about $P(A \cup B)$
  - What's the *worst case* value in terms of making $P(A \cap B)$ as small as possible?
  - By **normalization**: $P(A \cup B) = 1$
...
- But by **non-negativity**, the probability is also trivially bounded by $0$, so we have
